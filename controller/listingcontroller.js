...
} from "../validator/listingvalidator.js";
...
export const addListing = async (req, res) => {
  try {
    ...
    return res.json({
      message: "Request unsuccessful, kindly reload the page again",
      status: "error",
    });
  }
};
...
export const getOneListing = async (req, res) => {
  try {
    const { error, value } = listingIdValidator.validate(req.params, {
      abortEarly: false,
    });
...
export const updateListing = async (req, res) => {
  try {
    const { error, value } = updateListingValidator.validate(req.body, {
      abortEarly: false,
    });
    ...
    if (!confirmListing) {
      return res
        .status(400)
        .json({ message: "Listing not found", status: "error" });
    }
    ...
    const result = await ListingModel.findByIdAndUpdate(req.params.id, value, {
      new: true,
    });
...
export const replaceListing = async (req, res) => {
  try {
    ...
    if (confirmListing.userId.toString() !== req.auth.id) {
      return res
        .status(403)
        .json({ message: "You are not authorized to update this lisitng" });
    }
  ...
  try {
    ...
    if (req.params.userId !== req.auth.id) {
      return res
        .status(403)
        .json({
          message: "You are not authorized to view these deleted listings",
        });
    }
